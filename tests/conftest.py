...
def _cleanup_test_data():
    ...
    try:
        ...
        if _test_group_ids:
            ...
            cur.execute("DELETE FROM daily_reward_surges WHERE group_id IN %s", (ids,))
            cur.execute("DELETE FROM daily_reward_surge_overrides WHERE group_id IN %s", (ids,))
            cur.execute("DELETE FROM daily_reward_pricing WHERE group_id IN %s", (ids,))
            cur.execute("DELETE FROM daily_pricing_overrides WHERE group_id IN %s", (ids,))
            cur.execute("DELETE FROM coupons WHERE group_id IN %s", (ids,))
            cur.execute("DELETE FROM daily_medals WHERE group_id IN %s", (ids,))
